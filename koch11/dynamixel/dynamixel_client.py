...
from dynamixel_sdk import PortHandler, PacketHandler, GroupSyncRead, GroupSyncWrite, DXL_LOBYTE, DXL_HIBYTE, DXL_HIWORD, DXL_LOWORD
from enum import Enum
from typing import List

ControlTableElementDescription = namedtuple("ControlTableElement", ["address", "num_bytes", "is_read_only"])
...
class DynamixelXLSeriesClient:
  def __init__(self, port_name="/dev/ttyACM0", baud_rate=2000000):
    ...
    for elem in ControlTable:
      self.group_sync_read[elem] = GroupSyncRead(self.port_handler, self.packet_handler, elem.value.address, elem.value.num_bytes)
      self.group_sync_write[elem] = GroupSyncWrite(self.port_handler, self.packet_handler, elem.value.address, elem.value.num_bytes)
  ...
  def write(self, motor_id: int, control_table: ControlTable, data, retry_num=10):
    for _ in range(retry_num):
      try:
        match control_table.value.num_bytes:
          case 1:
            comm_result, error = self.packet_handler.write1ByteTxRx(self.port_handler, motor_id, control_table.value.address, data)
          case 2:
            comm_result, error = self.packet_handler.write2ByteTxRx(self.port_handler, motor_id, control_table.value.address, data)
          case 4:
            comm_result, error = self.packet_handler.write4ByteTxRx(self.port_handler, motor_id, control_table.value.address, data)
          case _:
    ...
    if comm_result != robotis_def.COMM_SUCCESS:
      raise RuntimeError(f"Communication failed because of {self.packet_handler.getTxRxResult(comm_result)}")
    if error != 0:
      raise RuntimeError(f"Write operation failed because of {self.packet_handler.getRxPacketError(error)}")
  ...
  def read(self, motor_id:int, control_table: ControlTable, retry_num=10):
    for _ in range(retry_num):
      try:
        match control_table.value.num_bytes:
          case 1:
            value, comm_result, error = self.packet_handler.read1ByteTxRx(self.port_handler, motor_id, control_table.value.address)
          case 2:
            value, comm_result, error = self.packet_handler.read2ByteTxRx(self.port_handler, motor_id, control_table.value.address)
          case 4:
            value, comm_result, error = self.packet_handler.read4ByteTxRx(self.port_handler, motor_id, control_table.value.address)
          case _:
            raise ValueError("Invalid num_bytes are detected")
      except IndexError:
        continue
    ...
    if comm_result != robotis_def.COMM_SUCCESS:
      raise RuntimeError(f"Communication failed because of {self.packet_handler.getTxRxResult(comm_result)}")
    if error != 0:
      raise RuntimeError(f"Read operation failed because of {self.packet_handler.getRxPacketError(error)}")
    
    return value

  def sync_read(self, motor_ids: List[int], control_table: ControlTable, retry_num=10):
    ...
    return [self.group_sync_read[control_table].getData(motor_id, control_table.value.address, control_table.value.num_bytes) for motor_id in motor_ids]
  
  def sync_write(self, motor_ids: List[int], control_table: ControlTable, data_list: List, retry_num=10):
    ...
    for motor_id, data in zip(motor_ids, data_list):
      match control_table.value.num_bytes:
        ...
        case 4:
          binary_data = [DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)), DXL_LOBYTE(DXL_HIWORD(data)), DXL_HIBYTE(DXL_HIWORD(data))]
        case _:
          raise ValueError(f"Invalid num_bytes {control_table.value.num_bytes} are detected")
  ...
  def reboot(self, motor_id: int):
    ...
    if comm_result != robotis_def.COMM_SUCCESS:
      raise RuntimeError(f"Communication failed because of {self.packet_handler.getTxRxResult(comm_result)}")
    if error != 0:
      raise RuntimeError(f"Reboot operation failed because of {self.packet_handler.getRxPacketError(error)}")
