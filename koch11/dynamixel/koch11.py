...
q_range = {
  'min': np.array([-np.pi / 2, -np.pi / 2, -np.pi / 2, -np.pi / 2, -np.pi / 2]),
  'max': np.array([np.pi / 2, np.pi / 2, np.pi / 2, np.pi / 2, np.pi / 2])
}
...
def make_client(port_name: str="/dev/ttyACM0", baud_rate=2000000) -> DynamixelRobotClient:
  return DynamixelRobotClient([1, 2, 3, 4, 5], dh_params, q_range, dq_range, port_name=port_name, baud_rate=baud_rate, retry_num=30)